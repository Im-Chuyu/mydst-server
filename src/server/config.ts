...
export const config = {
  ...
  root,
  sourceRoot: path.resolve(process.env.MYDST_SOURCE_DIR || "/opt/mydst-server"),
  panelRoot: process.cwd(),
  gameRoot: path.join(root, "game"),
  gameBinary64: path.join(root, "game", "bin64", "dontstarve_dedicated_server_nullrenderer_x64"),
  gameBinary32: path.join(root, "game", "bin", "dontstarve_dedicated_server_nullrenderer"),
  ...
  panelUpdateRequest: path.join(root, "panel-update.request"),
  panelUpdateState: path.join(root, "panel-update.state"),
