...
- 游戏日志、DST 控制台和在线玩家操作
- 首页房间状态、直连代码、聊天记录、快照回档、世界重置和存档删除
- 管理员、封禁和白名单管理
- tar.gz/ZIP 安全校验、上传、下载、恢复和保留策略
- Workshop MOD 名称/封面补全、下载状态和可视化配置
- SteamCMD 更新、定时备份和定时更新
- 密码哈希、HttpOnly 会话、CSRF 防护、登录限流和操作审计
- 首页用户可一键更新管理后台源码；计划任务和首页的“更新游戏服务端”只更新 DST 游戏文件
...
服务器通常优先使用 HTTPS 克隆；只有已经配置 GitHub SSH Key 时才使用 SSH：
...
如果服务器已经配置 GitHub SSH Key，也可以使用 SSH：
...
安装器会自动完成以下工作：安装系统依赖（包括 Git）和 SteamCMD 所需的兼容库、安装 Node.js 22、创建无特权 `dst` 用户、安装或更新 DST Dedicated Server、构建管理后台、创建 systemd 服务，并在 UFW 已启用时添加面板和 Steam 通信规则。启动游戏时会优先使用 DST 的 64 位 `bin64` 程序；只有服务器端没有 64 位程序时才回退到 `bin` 入口。
...
恢复上传的 tar.gz/ZIP 存档时，面板会读取 Master 和 Caves 中的 `leveldataoverride.lua`（已生成世界的实际世界配置）、`worldgenoverride.lua`（待生成世界配置）和 `modoverrides.lua`，将 Workshop MOD 和 Lua 配置同步到“MOD 管理”，并通过 SteamCMD 校验/续传每个 MOD 后再同步到游戏目录。大 MOD 单次下载最多等待 60 分钟并自动重试；若某个 MOD 不允许 SteamCMD 匿名预下载，面板会保留 `dedicated_server_mods_setup.lua` 下载配置，由 DST 分片在启动时继续自动下载。
...
|---|---|---:|---|---:|
| caves | TCP+UDP | 8432 | 服务器公网IP | 8432 |

执行上面的配置命令后，管理后台访问地址是 `http://服务器公网IP:8432`，同时可在后台把 Caves 游戏端口设置为 `8432`。Master 游戏端口对应的服务器商规则必须包含 UDP，只有 TCP 的规则不能用于 DST 地面世界连接。
...
如果 Ubuntu 的 UFW 处于 active 状态，安装器无法提前知道你之后在后台填写的 Master/Caves 端口，因此还需要在保存游戏端口后手动放行对应 UDP 端口。例如：

```bash
sudo ufw allow 8077/udp comment "MyDST Master"
sudo ufw allow 8432/udp comment "MyDST Caves"
```

如果面板 TCP 和 Caves UDP 使用相同数字端口，分别放行 TCP 和 UDP 即可；两种协议不会冲突。
...
如果只是重启服务器，而 `/opt/mydst/` 数据目录和 systemd 配置仍然保留，面板会由 systemd 自动启动。登录后先检查状态：

```bash
systemctl status mydst-panel --no-pager
```

如果状态不是 `active (running)`，再执行恢复命令：
...
如果系统被重新安装导致 `/etc/mydst-panel.env` 或 `/etc/systemd/system/mydst-panel.service` 不存在，即使 `/opt/mydst/` 数据仍保留，也需要重新执行一键安装以恢复系统依赖和服务配置；安装器会复用已有的游戏和数据文件。
...
```bash
cd /opt/mydst-server && git -c http.version=HTTP/1.1 pull --ff-only && sudo bash deployment/update.sh
```
...
首页的“更新管理后台”按钮执行同一套 Git 拉取和构建流程，普通用户和管理员均可使用。首次升级到包含该按钮的版本时，仍需手动执行一次上面的更新命令，让安装器把 systemd 更新触发器安装到系统；之后即可从首页发起后台更新。使用本地上传且没有 `.git` 目录的源码部署时，按钮会提示无法从 GitHub 更新，应继续使用手动更新方式。
