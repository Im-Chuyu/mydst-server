...
- 在服务器商控制台放行面板 TCP 端口以及下面的 DST UDP 端口；
...
```bash
cd /opt
git clone git@github.com:Im-Chuyu/-.git mydst-server
cd /opt/mydst-server
```
...
```bash
cd /opt
git clone https://github.com/Im-Chuyu/-.git mydst-server
cd /opt/mydst-server
```
...
安装器会自动完成以下工作：安装系统依赖和 32 位运行库、安装 Node.js 22、创建无特权 `dst` 用户、安装 SteamCMD、下载或更新 DST Dedicated Server、构建管理后台、创建 systemd 服务，并在 UFW 已启用时添加对应防火墙规则。

每台实例的端口由部署管理员在安装时确定。下面的示例将端口设置为一组新值：
...
`sudo -E` 用来保留前面设置的端口环境变量。安装完成后，普通用户不能在房间配置页面修改端口；管理员可以进入“系统设置 → 管理员端口”修改。安装时使用的端口配置保存在 `/opt/mydst/panel-ports.json`，恢复存档不会覆盖它。
...
curl http://127.0.0.1:8114/api/health
sudo -u dst env TMUX_TMPDIR=/opt/mydst/tmux tmux list-sessions
```

如果安装时将面板改成了其他端口，请把上面 `8114` 换成实际的 `MYDST_PANEL_PORT`。首次安装后没有自动创建游戏世界，登录后台配置 Cluster Token、房间和世界设置后，再从主页启动分片。
