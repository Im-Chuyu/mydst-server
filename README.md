...
- 首页管理员可一键更新管理后台源码；计划任务和首页的“更新游戏服务端”只更新 DST 游戏文件
...
管理员首页的“更新管理后台”按钮执行同一套 Git 拉取和构建流程。首次升级到包含该按钮的版本时，仍需手动执行一次上面的更新命令，让安装器把 systemd 更新触发器安装到系统；之后即可从首页发起后台更新。使用本地上传且没有 `.git` 目录的源码部署时，按钮会提示无法从 GitHub 更新，应继续使用手动更新方式。
